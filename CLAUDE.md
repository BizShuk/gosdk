...
- `MetricService.SendMultiContext(ctx, ...)` 讓呼叫端的取消與 deadline 生效, `SendMulti` 只是包一個 30s 背景 ctx 的相容入口; `Metric.Time` (ms 精度, remote write 的解析度) 非零時優先於秒級 `Timestamp`, 舊呼叫端不受影響
