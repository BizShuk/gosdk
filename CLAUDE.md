...
- 跨格式設定優先權（4 層）：`OS env (APP_*)` > `.env` > `settings.json` > `config.yaml`（高者覆蓋低者）。`config.Default()` 內 `loadAllConfigs()` 依序 merge `yaml → json → env`（後者覆蓋前者），最後呼叫 `viper.SetEnvPrefix("APP")` + `viper.AutomaticEnv()` + `bindAllEnvVars()` 啟用 OS env 動態查詢。**viper 限制與 workaround**：① 對 flat key（含底線）env 名稱 = `prefix + "_" + UPPER(key)`，所以 `viper.Get("app_name")` 查的是 `APP_APP_NAME`（prefix 與 key 疊加），不是 `APP_NAME`；② 對 nested key（如 `server.port`）`AutomaticEnv` 預設不生效，`bindAllEnvVars()` 透過 reflection 走完 `viper.AllSettings()` 並對每個 leaf 呼叫 `viper.BindEnv(key, "APP_"+UPPER(key))`，確保 nested key（含任意深度，如 `server.db.mysql.host`）也能被 OS env 覆寫（驗證見 `config/config_test.go:TestDefault_EnvVarOverridesAllFiles` + `TestDefault_NestedKeyEnvOverride`）
